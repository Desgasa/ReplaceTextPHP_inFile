...
<div class="alert" id="alert1" hidden>
        ...
        <strong>Помилка!<br></strong> Текст у файлі не знайдено.<br>Спробуйте ввести текст знову.
</div>
<div class="alert_success" id="alert2" hidden>
        <span class="closebtn">&times;</span>
        <strong>Успіх!<br></strong> Текст у файлі знайдено.<br>Прохання ввести новий текст.
</div>
<div class="alert_success2" id="alert3" hidden>
        <span class="closebtn">&times;</span>
        <strong>Успіх!<br></strong> Текст у файлі замінено.<br>Прохання перевірети виконання заміни у файлі .... за шляхом ....
</div>
...
<div id="second" class="replace" hidden>
    <form method="post" action="">
        <label>Текст на який замінюємо:</label><br>
        <textarea  type="text" id="replace" name="replace_string" class="txta"><?php echo htmlspecialchars("");
            if(isset($_POST['finish'])){
                $replace = $_POST['replace_string'];
                $replacer = str_replace(PHP_EOL, '\\n', $replace);
                echo htmlspecialchars($replacer);
            }?></textarea><br>
        <button type="submit"  name="finish"  id="button_replace" class="btn_replace">Замінити текст</button><br>
    </form>
</div>
...
<div class="pop_up" id="pop_up">
	<div class="pop_up_container">
		<div class="pop_up_body" id="pop_up_body">
			<h2>Допомога</h2>
			<p>На даному етапі Вам потрібно ввести текст, який Ви бажаєте замінити, якщо текст буде знайдено у Вас з'яветься поле в якому необхідно буде вести новий текст.</p>
...
copy('C:/Test/TG_Test.json', $newDir . '/Old_File/VB_Test_' .date("d.m.Y").'.json');
copy('C:/Test/TG_Test.json', $newDir . '/New_File/VB_Test.json');
?>
<?php
if(isset($_POST['start'])) {
    ...
    $find = $_POST['search_string'];
    $findr = str_replace(PHP_EOL, '\\n', $find);
    $file = file_get_contents($newDir . '/New_File/VB_Test.json');
...
if(isset($_POST['finish'])) {
    ...
    $file = $newDir . '/New_File/VB_Test.json';
    $file_contents = file_get_contents($file);
    $file_contents = str_replace($findr, $replacer, $file_contents);
